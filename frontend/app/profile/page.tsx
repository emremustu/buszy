      ...
      try {
        ...
        if (result.success) {
          localStorage.removeItem('userMail');
            ...
            localStorage.removeItem('userId'); // Assuming result contains user_id
            localStorage.removeItem('name');
            localStorage.removeItem('account_type');

            localStorage.removeItem('rememberMe' );
            sessionStorage.removeItem('userLoggedIn');
            sessionStorage.removeItem('userId'); // Assuming result contains user_id
          ...
          alert(result.message); // Hata mesajını burada göster
        }
      }
      catch (error) {
        ...
        {loading ? (
          ...
          <div className="w-full max-w-5xl flex flex-col items-center gap-10">
            ...
            <div className="flex justify-between w-full">
              <div></div>
              <button onClick={handleDeleteAccount} className="bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded flex items-center gap-2 cursor-pointer">
