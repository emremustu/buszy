  ...
  const handleDeleteTicket = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/delete-ticket', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ticket_id: ticketToDelete }),
      });

      const result = await response.json();
      if (result.success) {
        setTickets((prev) => prev.filter((ticket) => Number(ticket[0]) !== ticketToDelete));
        setShowDeleteModal(false);
        setTicketToDelete(null);
      } else {
        alert(result.message || 'Failed to delete the ticket.');
      }
    } catch (err) {
                  ...
                  {hasDeparted ? (
                    submitted[ticketId] ? (
                      <div className="w-full text-center">
                        <p className="text-gray-700 font-semibold">Your Comment:</p>
                        ...
                        {ratings[ticketId] && (
                          <div className="mt-3 w-full">
                            <textarea
                              maxLength={255}
                              rows={3}
                              placeholder="Write your comment (max 255 characters)"
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm">
            ...
            <p className="mb-4">You're about to cancel this ticket. This action cannot be undone.</p>
